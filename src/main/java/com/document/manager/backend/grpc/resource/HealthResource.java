...
public class HealthResource {
   ...
   * /api/health/check?service=service.grpc.com.document.manager.backend.FileInfoService GET /api/health/check
  ...
  @GET
  @Path("/check")
  public Uni<HealthResponse> checkHealth(@QueryParam("service") final String service) {

    logger.infof(
        "HTTP health check request for service: %s", service != null ? service : "overall");

    final String serviceName = service != null ? service : "";
  ...
  public static class HealthResponse {
    ...
    HealthResponse(final String status) {
      this.status = status;
    }
  }
}
