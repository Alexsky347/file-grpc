...
/**
 * Example of how to call the Health gRPC service.
 * This demonstrates both the Check (unary) and Watch (server streaming) RPC calls.
 */
@Singleton
public class HealthServiceClient {
     ...
     * Usage:
     *
     * @Inject HealthServiceClient client;
     * <p>
     * // In your method:
     * client.checkHealth("com.drive.backend.grpc.service.FileInfoService")
     * .subscribe().with(
     * response -> logger.infof("Service status: %s", response.getStatus()),
    ...
    /**
     * Example: Call the Watch method to get a continuous stream of health status updates.
     * The stream updates every 5 seconds.
     * <p>
     * Usage:
     *
     * @Inject HealthServiceClient client;
     * <p>
     * // In your method:
     * client.watchHealth("com.drive.backend.grpc.service.FileInfoService")
     * .subscribe().with(
     * response -> logger.infof("Service status update: %s", response.getStatus()),
     ...
     * <p>
     * // In your method:
     * client.checkOverallHealth()
     * .subscribe().with(
     * response -> logger.infof("Overall server status: %s", response.getStatus()),
     * failure -> logger.errorf(failure, "Overall health check failed")
     * );
     */
