...
/**
 * REST endpoint to expose Health gRPC service via HTTP.
 * This allows HTTP clients to check the health status of services.
 */
...
public class HealthResource {
     ...
     * <p>
     * Example requests:
     * GET /api/health/check?service=com.drive.backend.grpc.service.FileInfoService
     * GET /api/health/check (checks overall server health)
     *
    ...
    @GET
    @Path("/check")
    public Uni<HealthResponse> checkHealth(
            @QueryParam("service") String service) {

        logger.infof("HTTP health check request for service: %s", service != null ? service : "overall");
    ...
    /**
     * Simple HTTP endpoint for basic liveness probe.
    ...
    /**
     * Simple HTTP endpoint for readiness probe.
