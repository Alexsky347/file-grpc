...
public class FileInfoService implements FileService {
  ...
  /**
   * This service implements the FileService interface generated by the Mutiny Grpc generator. It
   * provides an implementation for the uploadFile method.
   */
  @RolesAllowed({USER, ADMIN})
  @Override
  @RunOnVirtualThread
  public Uni<io.quarkus.example.FileUploadResponse> uploadFile(
      io.quarkus.example.FileUploadRequest request) {
        ...
        .transformToUni(
            user -> {
              if (user == null)
                return Uni.createFrom()
                    .item(
                        io.quarkus.example.FileUploadResponse.newBuilder()
                            .setSuccess(false)
                            .setMessage("User not authenticated")
                            .build());
              return minioService
                  .uploadFile(
                      Base64.getDecoder().decode(request.getContent()), request.getFilename(), user)
                  .onItem()
                  .transform(
                      objectName -> {
  ...
  @RunOnVirtualThread
  @Override
  public Uni<io.quarkus.example.FileOperationResponse> deleteFile(
      io.quarkus.example.FileDeleteRequest request) {
    String uuid = request.getUuid();

    return Uni.createFrom()
        .item(this::getUsername)
        .onItem()
        .transformToUni(
            user ->
                minioService
                    ...
                    .transformToUni(
                        files -> {
                          ...
                          if (objectName == null)
                            return Uni.createFrom()
                                .item(
                                    io.quarkus.example.FileOperationResponse.newBuilder()
                                        .setSuccess(false)
  ...
  @RolesAllowed({USER, ADMIN})
  @Override
  public Uni<io.quarkus.example.FileOperationResponse> renameFile(
      io.quarkus.example.FileRenameRequest request) {
    String uuid = request.getUuid();
                            ...
                            return Uni.createFrom()
                                .item(
                                    io.quarkus.example.FileOperationResponse.newBuilder()
                                        ...
                                        .build());
                          return minioService
                              .renameFile(oldObjectName, newFilename, user)
                              .onItem()
                              .transform(
                                  success ->
                                      io.quarkus.example.FileOperationResponse.newBuilder()
  ...
  /**
   * Implementation of the listUserFiles method from the FileService interface. Lists all files for
   * a specific user from MinIO storage.
   */
  @RunOnVirtualThread
  @RolesAllowed({USER, ADMIN})
  @Override
        ...
        .transformToUni(
            username ->
                minioService
                    ...
                    .transform(
                        files -> {
                          ListUserFilesResponse.Builder responseBuilder =
                              ListUserFilesResponse.newBuilder()
                                  .setSuccess(true)
                                  .setMessage("Files retrieved successfully for user: " + username);

                          // Convert from DTO FileInfo to gRPC FileInfo
                          for (FileInfoDto file : files) {
                            io.quarkus.example.FileInfo fileInfo =
                                io.quarkus.example.FileInfo.newBuilder()
